...
#define MX51_ECSPI_INT_RDREN		BIT(4)		// (1 << 4)
#define MX51_ECSPI_INT_TCEN		BIT(7)		// (1 << 7)
...
// Added to read data from RXDATA register
static void mx53_ecspi_rx_slave(struct spi_imx_data *spi_imx)
{
	u32 val = be32_to_cpu(readl(spi_imx->base + MXC_CSPIRXDATA));		// Read the data from ECSPIx_RXDATA
...
// Added to write data to TXDATA register
static void mx53_ecspi_tx_slave(struct spi_imx_data *spi_imx)
{
	...
	writel(val, spi_imx->base + MXC_CSPITXDATA);				// Write data to the ECSPIx_TXDATA
}
...
//	ctrl |= MX51_ECSPI_CTRL_MODE_MASK;
	...
	printk("mx51_ecspi_config");

	/* set Master or Slave mode */
	if (spi_imx->slave_mode)
		ctrl &= ~MX51_ECSPI_CTRL_MODE_MASK;		// Clear all Channel Mode bits to enable SPI Slave mode
	else
		ctrl |= MX51_ECSPI_CTRL_MODE_MASK;
...
//	cfg |= MX51_ECSPI_CONFIG_SBBCTRL(config->cs);
	 ...
	 */
	if (spi_imx->slave_mode && is_imx53_ecspi(spi_imx))		// Clear SS_CTL Wave Form Select to support Burst
		cfg &= ~MX51_ECSPI_CONFIG_SBBCTRL(config->cs);		// Transfer in Slave Mode
	else
		cfg |= MX51_ECSPI_CONFIG_SBBCTRL(config->cs);
...
static int spi_imx_probe(struct platform_device *pdev)
{
	...
	if (of_get_property(np, "fsl,spi-slave-mode", NULL) && is_imx5x_ecspi(spi_imx))
	...
	if (!spi_imx->slave_mode && spi_imx->devtype_data == &imx51_ecspi_devtype_data
